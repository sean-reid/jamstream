...
/// a row measured against anything lower than this edge can be well past the fold
/// and still read as on screen.
...
fn lowest_row(harness: &Harness<'_>) -> egui::Rect {
    harness
        .get_by_role_and_label(AkRole::Window, "Settings")
        .children_recursive()
        .filter(|node| node.accesskit_node().raw_bounds().is_some())
        .map(|node| node.rect())
    ...
    for size in drawer_sizes() {
        for tab in [
            ...
            assert!(
                ...
                "the end of the {label} panel at {last:?} stays below the drawer's body at \
                 {body:?}, window {size:?}"
            );
        }
    }
