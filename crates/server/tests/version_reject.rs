...
//! protocol version and gets told so, verifiably. A crafted init datagram
//! with version 2 hits a real server socket; the reply must parse as a
//! VersionReject whose MAC verifies under the server key an invite carries,
//! and a forged reject MACed with the wrong key must not verify and must be
//! ignored by a real client core. The deterministic rate-limit coverage
//! lives in crates/session/tests/loopback.rs.
...
async fn wrong_version_init_gets_a_mac_verified_reject() {
    ...
    // A handshake init claiming version 2 against a version-1 server.
    let wrong_version = PROTOCOL_VERSION + 1;
    let init = wire::build_handshake_init(wrong_version, &[0x5A; 48]);
    ...
    // It MAC-verifies under the server public key an invite would carry,
    // bound to the exact init it answers.
    assert!(
        wire::verify_version_reject(&server_keys.public, ours, theirs, &mac, &init),
        "reject must verify under the invite's server key"
    );
    // Bound to this init: the same reject does not vouch for another one.
    let other_init = wire::build_handshake_init(wrong_version, &[0x77; 48]);
    ...
    // A forgery MACed with any other key fails verification...
    let forged = wire::build_version_reject(&[0xEE; 32], ours, theirs, &init);
    ...
    // change, no event, no reply. Only the true server key can tell a
    // client to give up.
    ...
    let forged_for_client =
        wire::build_version_reject(&[0xEE; 32], PROTOCOL_VERSION, PROTOCOL_VERSION, &sent_init);
