...
use jamstream_audio_io::{CallbackBridge, DeviceSide, EngineSide, playout_cushion_samples};

use crate::scenario::STEREO_FRAME;
...
pub struct PlayoutDevice {
    device: DeviceSide,
    engine: EngineSide,
    /// The depth the top-up loop holds, which the client's worker holds too:
    /// [`playout_cushion_samples`] is the one definition of it.
    target: usize,
...
impl PlayoutDevice {
    ...
    pub fn new(device_frames: u32) -> PlayoutDevice {
        ...
        let target = playout_cushion_samples(callback, STEREO_FRAME);
