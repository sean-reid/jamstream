...
One page per provider walks the account and credential creation from zero. Hosting on your own computer needs none of this; see [Playing on the same network](local.md).
...
One credential per provider covers everything except one feature. [Recording a cloud session](recording.md) writes takes to a bucket, which needs a second key, scoped to that one bucket and nothing more.
