...
The headline number on the left of the status bar is mouth to ear latency in milliseconds: from sound entering your interface to a bandmate's sound leaving theirs, measured, not estimated. Under 20 ms most people stop noticing; under 30 ms feels like playing across a large stage. Beside it are your input and output meters. Hover the number for the rest of the link: `rtt`, your round trip to the server; `buffer`, the jitter buffer depth in 2.5 ms frames, which climbs when your network jitters; and `loss`, the packet loss percentage, which should sit near 0.0%. The middle of the bar carries ON AIR and REC, the two states with consequences outside the room. Hosts also see elapsed time and cost so far on the right. [Troubleshooting](troubleshooting.md#latency-feels-high) turns each number into an action.
