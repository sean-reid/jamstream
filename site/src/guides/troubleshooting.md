...
4. **Sample rate.** A device that does not run at 48 kHz plays through a converter that adds about 3 ms per converted direction. While that happens the device's rate sits beside the latency number (a muted "44.1 kHz"), the number already includes the cost, and the hover names each converted direction with its milliseconds. To remove the cost, set the device itself to 48 kHz in the system's sound settings; see [Device problems](#device-problems) for the walk on each platform.
