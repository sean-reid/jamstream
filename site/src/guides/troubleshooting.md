...
4. **WASAPI mode, on Windows.** In exclusive mode the device adds about 10 ms; shared mode adds 20 to 30 ms. The latency number's hover says which one this session got, and whether the OS is resampling playback to the device's own rate, which costs more still. To make exclusive available, tick "Allow applications to take exclusive control of this device" on the device's Advanced tab in the Sound dialog, for both the input and the output.
